...
# Define common activation functions using NumPy for numerical evaluation
# and Sympy for symbolic manipulation.

def linear(x_val):
    """Linear activation function."""
    # This function is used both numerically and symbolically.
    # If x_val is a Sympy Symbol, it should return the symbol itself for symbolic processing.
    # If x_val is a number/Numpy array, it should return the number/array.
    return x_val

def sigmoid(x_val):
    """Sigmoid activation function."""
    if isinstance(x_val, sympy.Expr) or isinstance(x_val, sympy.Symbol):
        return 1 / (1 + E**(-x_val))
...
def tanh(x_val):
    """Hyperbolic tangent activation function."""
    if isinstance(x_val, sympy.Expr) or isinstance(x_val, sympy.Symbol):
        return sympy.tanh(x_val)
    return np.tanh(x_val)

def softplus(x_val):
    """Softplus activation function."""
    if isinstance(x_val, sympy.Expr) or isinstance(x_val, sympy.Symbol):
        return sympy_log(1 + E**x_val) # Use sympy.log for symbolic expressions
...
def get_taylor_coefficients(func_name_str, order, around=0.0):
    """
    Calculates Taylor series coefficients for a given function name, expanded around a point.
    The coefficients c_k are such that f(x) = sum_{k=0 to order} c_k * (x-around)^k.
    This means c_k = f^(k)(around) / k!.

    Args:
        func_name_str (str): Name of the function (e.g., 'tanh', 'sigmoid', 'linear', 'softplus').
        order (int): The maximum order of the Taylor expansion (degree of polynomial).
        around (float, optional): The point around which to expand. Defaults to 0.0.

    Returns:
        numpy.ndarray: A 1D NumPy array of Taylor coefficients [c0, c1, ..., c_order],
                       where c_k = f^(k)(around) / k!. The length of the array is order + 1.
    """
    if not isinstance(order, int) or order < 0:
        raise ValueError("Order must be a non-negative integer.")
    if not isinstance(around, (int, float)):
        raise ValueError("The 'around' point must be a numerical value.")

    if func_name_str not in _ACTIVATION_FUNCTIONS_MAP:
        raise ValueError(f"Unknown activation function: {func_name_str}")

    x = Symbol('x') # Sympy symbol for differentiation
    
    # Get the symbolic representation of the function
    # For 'linear', _ACTIVATION_FUNCTIONS_MAP['linear'](x) will return x itself.
    # For others, it will return their sympy expressions.
    func_symbolic = _ACTIVATION_FUNCTIONS_MAP[func_name_str](x)

    coeffs = []
    
    # Special handling for linear as its derivatives are very simple
    if func_name_str == 'linear':
        # f(x) = x. Taylor expansion around 'a': f(x) = a + 1*(x-a) + 0*(x-a)^2/2! + ...
        # c0 = f(a) = a
        # c1 = f'(a)/1! = 1
        # c_k = f^(k)(a)/k! = 0 for k >= 2
        coeffs.append(float(around)) # c0 = a
        if order >= 1:
            coeffs.append(1.0)       # c1 = 1
        for _ in range(2, order + 1): # c2, c3, ...
            coeffs.append(0.0)
    else:
        # General case for non-linear functions using sympy
        current_deriv_expr = func_symbolic
        for k_deriv_order in range(order + 1):
            # Evaluate f^(k)(around)
            val_at_around = current_deriv_expr.subs(x, around).evalf()
            
            # Taylor coefficient c_k = f^(k)(around) / k!
            coeffs.append(float(val_at_around / factorial(k_deriv_order)))
            
            # Differentiate for the next iteration, if needed
            if k_deriv_order < order:
                current_deriv_expr = sympy.diff(current_deriv_expr, x)
